...
const AI_SERVICE_PRESETS = {
    cstcloud: { name: 'CSTCloud Uni-API', endpoint: 'https://uni-api.cstcloud.cn/v1/chat/completions', model: 'deepseek-v3', protocol: 'openai-compatible' }
};
    ...
    loadAIConfig() {
        ...
        if (aiConfig) {
            try {
                ...
                this.currentService = config.currentService || 'cstcloud';
        ...
        if (oldDeepSeekKey && !this.services.cstcloud.apiKey) {
            console.log('🔄 发现旧的DeepSeek配置，迁移中...');
            this.services.cstcloud.apiKey = oldDeepSeekKey;
            this.services.cstcloud.enabled = true;
            this.currentService = 'cstcloud';
            this.saveAIConfig();
        }
    }
