...
- macOS 小组件：独立于浏览器运行，启动时显示为主屏幕左侧的悬浮图标，点击图标展开或收起完整面板，普通应用打开后会自然覆盖它。
...
模型名称可在界面修改。API Key 通过本机设置保存，并由前端安全存储模块加密保存；桌面版通过 Electron 本地进程向 CSTCloud 发起固定地址请求，避免浏览器跨域限制。
...
- 启动后只显示四象限图标；点击图标展开完整面板，再次点击同一图标即可收起。
- 顶部只保留一个设置按钮，用于配置和测试 AI 连接。
- 菜单栏图标可以打开完整面板；右键菜单可重新定位、折叠/展开、设置开机启动或退出。
...
```text
dist/Productivity Tool-2.0.2-arm64.dmg
```
...
接收者打开 DMG 后，把应用拖进“应用程序”文件夹即可。安装包不会携带你的 API Key，接收者需要在应用顶部设置按钮中填写自己的 Key。
...
5. 填写 API Key，点击“保存并测试”；界面会直接提示连接、Key、模型或余额状态。
