    ...
    async loadAIConfig() {
        ...
        if (aiConfig) {
            try {
                ...
                this.currentService = aiConfig.currentService || 'cstcloud';
                Object.keys(this._services).forEach(serviceName => {
                    if (aiConfig[serviceName]?.model) {
                        this._services[serviceName].model = aiConfig[serviceName].model;
                    }
                });
        ...
        // 将旧版 DeepSeek Key 平滑迁移到统一网关。
        const oldDeepSeekKey = window.DataSyncStorage.getRaw('deepSeekApiKey');
        if (oldDeepSeekKey && !this._services.cstcloud.apiKey) {
            debugLog('🔄 发现旧的DeepSeek配置，迁移中...');
            this._services.cstcloud.apiKey = oldDeepSeekKey;
            this._services.cstcloud.enabled = true;
            this.currentService = 'cstcloud';
