...
class FortuneSystem {
    ...
    loadAISettings() {
        ...
        if (savedApiKey) {
            ...
            if (window.aiServiceManager) {
                window.aiServiceManager.setAPIKey('cstcloud', savedApiKey).then(() => {
                    window.DataSyncStorage.removeRaw('deepSeekApiKey');
                    SafeLogger.debug('✅ 明文 API Key 已迁移并删除');
                });
            } else if (window.secureStorage) {
                window.secureStorage.ready().then(async () => {
                    let config = await window.secureStorage.getSecure('aiConfig');
                    if (!config) {
                        config = {
                            cstcloud: { enabled: true, apiKey: savedApiKey, model: 'deepseek-v3' },
                            currentService: 'cstcloud'
                        };
                    } else {
                        config.cstcloud = { enabled: true, apiKey: savedApiKey, model: 'deepseek-v3' };
                        config.currentService = 'cstcloud';
                    }
    ...
    async getApiKey() {
        ...
        if (window.aiServiceManager && window.aiServiceManager._services) {
            ...
            const key = window.aiServiceManager._services.cstcloud?.apiKey;
            if (key) {
        ...
        if (window.secureStorage) {
            ...
            if (config && config.cstcloud?.apiKey) {
                SafeLogger.debug('✅ 从 aiConfig 获取到API Key');
                return config.cstcloud.apiKey;
            }
    ...
    async getConfiguredAIServiceName() {
        ...
        if (legacyKey && window.aiServiceManager && typeof window.aiServiceManager.setAPIKey === 'function') {
            try {
                await window.aiServiceManager.setAPIKey('cstcloud', legacyKey);
        ...
        return legacyKey ? 'cstcloud' : null;
    }
    ...
    async generateAIFortune() {
        ...
        if (!aiServiceName) {
            ...
            document.getElementById('workbench-ai-settings')?.click();
            return;
        }

        if (!this.aiEnabled) {
            SafeLogger.debug('AI功能未启用');
            showNotification('请先在 AI 设置中保存配置', 'error');
            return;
        }
